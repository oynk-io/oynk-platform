...
export async function startPhoneChallenge(phoneInput: unknown, ip?: string) {
  ...
  const linked = await pool.query('SELECT wallet FROM consumer_phone_links WHERE phone=$1 AND client_id=$2 LIMIT 1', [phone, funding.SOCKETFI_CLIENT_ID]);
  ...
  return { challengeId: id, accountExists: linked.rows.length > 0, expiresAt: expires.toISOString(), resendAvailableAt: resend.toISOString(), destinationHint: `${phone.slice(0, 7)}••••${phone.slice(-2)}`, ...(config.APP_ENV === 'development' && !config.AFRICASTALKING_API_KEY ? { developmentCode: code } : {}) };
}
...
export async function verifyPhoneChallenge(challengeInput: unknown, codeInput: unknown) {
  ...
  try {
    ...
    return { phone: row.phone, verifiedAt: new Date().toISOString() };
...
export async function linkVerifiedPhone(identity: Identity, challengeInput: unknown) {
  ...
  const result = await pool.query('SELECT phone,consumed_at FROM consumer_phone_challenges WHERE id=$1', [id.data]);
  const challenge = result.rows[0];
  if (!challenge?.consumed_at) throw new PhoneAuthError('Verify your phone number before continuing.', 401);
  try {
    const existing = await pool.query('SELECT subject,network,wallet FROM consumer_phone_links WHERE phone=$1 AND client_id=$2', [challenge.phone, identity.clientId]);
    ...
    if (!existing.rows[0]) await pool.query(`INSERT INTO consumer_phone_links(phone,subject,client_id,network,wallet) VALUES($1,$2,$3,$4,$5)`, [challenge.phone, identity.subject, identity.clientId, identity.network, identity.wallet]);
