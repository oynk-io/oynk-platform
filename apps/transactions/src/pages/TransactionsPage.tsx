...
export function TransactionsPage() {
  ...
  const actions = <label><span className="sr-only">Filter transactions by settlement route</span><select value={chain} onChange={(event) => setChain(event.target.value as ChainFilter)} className="control"><option value="ALL">All settlement routes</option><option value="OFFCHAIN">Offchain settlement</option><option value="BSC">Onchain · {CHAIN_METADATA.BSC.label}</option><option value="SOLANA">Onchain · {CHAIN_METADATA.SOLANA.label}</option></select></label>;

  return <ConsoleShell title="Transactions" description="Review public-safe onchain and offchain settlement cash flow." actions={actions}>
    <div className="console-data-notice" role="note"><strong>Settlement activity</strong><span>Onchain records link to public explorers. Offchain identities are fully masked and open in a privacy-safe details view.</span></div>
