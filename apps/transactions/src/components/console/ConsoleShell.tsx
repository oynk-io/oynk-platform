...
export function ConsoleShell({ title, description, children, actions }: { title: string; description: string; children: ReactNode; actions?: ReactNode }) {
  ...
  return <div className={`console-shell ${collapsed ? "is-collapsed" : ""}`}>
    ...
    <aside className="console-sidebar" aria-label="Oynk workspace">
      <div className="console-brand"><a href={publicSiteUrl} aria-label="Oynk website"><BrandMark /><strong>Oynk</strong></a><button type="button" onClick={() => setCollapsed((current) => !current)} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}><ChevronLeft size={16} className={collapsed ? "rotate-180" : ""} /></button></div>
      ...
      <a className="console-docs-link" href="https://docs.oynk.io" target="_blank" rel="noopener noreferrer"><BookOpen size={17} aria-hidden="true" /><span>Documentation</span></a>
    </aside>
