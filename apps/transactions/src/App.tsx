...
export function DashboardPage() {
  ...
  return (
    <ConsoleShell title="Settlement activity" description="Verified transaction activity reflecting founder-led cross-border operations and Oynk’s growing indexed settlement coverage." actions={pageActions}>
        ...
        {loading && !data ? <DashboardSkeleton /> : data && (
          <>
            <div className="mb-5 rounded-xl border border-sky-300/15 bg-sky-300/[.06] px-4 py-3 text-xs leading-5 text-slate-400" role="note"><strong className="text-slate-300">Operating activity context.</strong> Figures reflect recorded operating activity and indexed transactions. They should not be interpreted as Oynk revenue, customer deposits, or total network volume.</div>

            <section aria-labelledby="metrics-heading"><h2 id="metrics-heading" className="sr-only">Demonstrated settlement activity metrics</h2><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
