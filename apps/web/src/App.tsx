        ...
        {loading && !data ? <DashboardSkeleton /> : data && (
          <>
            <section aria-labelledby="metrics-heading"><h2 id="metrics-heading" className="sr-only">Indexed activity metrics</h2><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <MetricCard label="Total volume" value={formatUsd(data.metrics.grossTransferVolumeUsd ?? data.metrics.totalUsd)} sub="All indexed on-chain movement" icon={Activity} />
            ...
            <dl className="console-detail-metrics" aria-label="Network activity details">
              <div><dt>Tracked wallets</dt><dd>{data.metrics.activeWallets.toLocaleString()}</dd></div>
              <div><dt>BSC volume</dt><dd>{formatUsd(data.metrics.bscUsd)}</dd></div>
              <div><dt>Solana volume</dt><dd>{formatUsd(data.metrics.solanaUsd)}</dd></div>
              <div><dt>Estimated settled</dt><dd>{formatUsd(data.metrics.estimatedSettledVolumeUsd ?? 0)}</dd></div>
            </dl>
