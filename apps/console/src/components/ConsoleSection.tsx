...
import { getConsoleRoutes, type ConsoleRoute } from "../consoleRoutes";
...
export function ConsoleSection({ organization, route }: { organization: OrganizationSummary; route: ConsoleRoute }) {
  ...
  return <>
    ...
    {isHome ? <section className="workspace-shortcuts" aria-label="Workspace shortcuts">{getConsoleRoutes(organization.type).filter(item => !item.path.endsWith('/home')).slice(0,3).map(item => <a key={item.path} href={item.path}><div><h2>{item.label}</h2><p>{item.description}</p></div><ArrowUpRight size={19}/></a>)}</section> : null}
