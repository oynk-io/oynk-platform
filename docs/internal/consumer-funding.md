...
5. Optionally configure `OYNK_NGN_PER_USDC` and `OYNK_RATE_EXPIRES_AT` (ISO timestamp). A blank or expired rate produces “Not available”, never a fabricated live quote. The value is an indicative estimate, not an executable conversion commitment. Rate values supplied in product examples are not installed as defaults.
